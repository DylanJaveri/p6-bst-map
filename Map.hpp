...
private:
  ...
  class PairComp {
    public:
    bool operator() (const std::pair<Key_type, Value_type> &lhs, const std::pair<Key_type, Value_type> &rhs) const{
    Key_compare compare;
...
public:
  ...
  Map() : tree(BinarySearchTree<Pair_type, PairComp>()) {};
  ...
  Iterator find(const Key_type& k) const {
    std::pair<Key_type, Value_type> p1 = {k, Value_type()};
    return tree.find(p1);
  }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val) {
    if (tree.find(val) != tree.end()){
      return  std::pair<Iterator, bool> {tree.find(val), false};
    }
    else {
